...
async function loadHomeData() {
    ...
    if (holdersEl) {
        holdersEl.textContent = 'Loading...';  // Better UX than static N/A
    }
    ...
    try {
        ...
        // ────────────────────────────────────────────────────────────────
        // Fetch Holders FIRST (since contract is verified and has public totalHolders)
        // ────────────────────────────────────────────────────────────────
        console.log('Fetching totalHolders...');
        const holdersRaw = await tokenContract.totalHolders();  // calls the public view function
        console.log('Total holders raw:', holdersRaw.toString());

        if (holdersEl) {
            // Simple comma formatting for nice display
            holdersEl.textContent = holdersRaw.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
        }

        // Total Supply (still hardcoded, but you can make dynamic later)
