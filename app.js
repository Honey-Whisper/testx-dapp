...
if (network.chainId !== 84532) {
    console.log('Wrong network detected. Current:', network.chainId, 'Expected: 84532');
...
console.log('Network correct, initializing contracts...');
console.log('Contract addresses:', CONFIG.CONTRACTS);
initializeContracts();

// Verify contracts initialized
...
async function loadHomeData() {
    ...
    // Always set total supply first
    const totalSupplyEl = document.getElementById('totalSupply');
    if (totalSupplyEl) {
        totalSupplyEl.textContent = '2,500,000';
    }
    
    // Check if contracts are initialized
    if (!tokenContract || !stakingContract) {
        console.error('❌ Contracts not initialized');
        document.getElementById('totalStaked').textContent = 'Connect Wallet';
        document.getElementById('circulatingSupply').textContent = 'Connect Wallet';
        return;
    }
    
    console.log('✓ Contracts initialized');
    console.log('✓ Token Contract:', tokenContract.contract.address);
    console.log('✓ Staking Contract:', stakingContract.contract.address);
    
    // Try to load staking data
    try {
        console.log('Fetching total staked from contract...');
        const totalStaked = await stakingContract.getTotalStaked();
        console.log('✓ Total staked retrieved:', totalStaked.toString());
        
        const formattedStaked = Utils.formatTokenAmount(totalStaked);
        console.log('✓ Formatted staked:', formattedStaked);
        
        document.getElementById('totalStaked').textContent = formattedStaked;
        
        // Calculate circulating supply
        const totalSupply = ethers.utils.parseEther('2500000');
        const circulating = totalSupply.sub(totalStaked);
        const formattedCirculating = Utils.formatTokenAmount(circulating);
        
        console.log('✓ Circulating supply:', formattedCirculating);
        document.getElementById('circulatingSupply').textContent = formattedCirculating;
        ...
        console.error('❌ ERROR LOADING STAKING DATA');
        ...
        if (error.code === 'CALL_EXCEPTION') {
            console.error('⚠️ CALL_EXCEPTION: Contract function call failed');
            console.error('Possible reasons:');
            console.error('1. Contract not deployed at this address');
            console.error('2. Function name mismatch in ABI');
            console.error('3. Wrong network');
            
            alert('Contract call failed! Check:\n' +
                  '1. Are contracts deployed on Base Sepolia?\n' +
                  '2. Are contract addresses correct?\n' +
                  '3. Check browser console for details');
        }
        
        // Set fallback values
        document.getElementById('totalStaked').textContent = '0';
        document.getElementById('circulatingSupply').textContent = '2,500,000';
        document.getElementById('holders').textContent = 'N/A';
    }
}
