...
const expectedChainId = parseInt(CONFIG.CURRENT_NETWORK.chainId, 16);
...
async function loadHomeData() {
    try {
        ...
        try {
            ...
            document.getElementById('totalSupply').textContent = formatTokenAmount(totalSupply);
            document.getElementById('totalStaked').textContent = formatTokenAmount(totalStaked);
            
            const circulating = totalSupply.sub(totalStaked);
            document.getElementById('circulatingSupply').textContent = formatTokenAmount(circulating);
        } catch (error) {
