...
async function loadData() {
    ...
    try {
        ...
        updateCharts(soldNum, stake.amount);
    } catch (e) {
...
document.getElementById('claimTokensBtn').onclick = async () => {
    try { const tx = await contract.claimPresaleTokens(); await tx.wait(); alert("Tokens claimed!"); loadData(); } catch (e) { alert("Failed: " + e.message); }
};
...
document.getElementById('claimRewardsBtn').onclick = async () => {
    try { const tx = await contract.claimRewards(); await tx.wait(); alert("Rewards claimed!"); loadData(); } catch (e) { alert("Failed: " + e.message); }
};

document.getElementById('claimOwnerBtn').onclick = async () => {
    try { const tx = await contract.claimOwnerVested(); await tx.wait(); alert("Owner vested claimed!"); loadData(); } catch (e) { alert("Failed: " + e.message); }
};

document.getElementById('claimGrowthBtn').onclick = async () => {
    try { const tx = await contract.claimGrowthVested(); await tx.wait(); alert("Growth vested claimed!"); loadData(); } catch (e) { alert("Failed: " + e.message); }
};
