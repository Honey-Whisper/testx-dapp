...
async function loadHomeData() {
    ...
    // Removed forced 'N/A' here — we will set it only if fetch fails
    // const holdersEl = document.getElementById('holders');
    // if (holdersEl) {
    //     holdersEl.textContent = 'N/A';  // Safe default — commented out
    // }

    try {
        if (!tokenContract || !stakingContract) {
            console.error('Contracts not initialized');
            // Only set Holders to N/A if contracts missing
            const holdersEl = document.getElementById('holders');
            if (holdersEl) holdersEl.textContent = 'N/A';
            return;
        }
        
        // Total Supply (hardcoded since it's fixed)
        const totalSupplyEl = document.getElementById('totalSupply');
        if (totalSupplyEl) {
            totalSupplyEl.textContent = '2,500,000';
        }
        
        // Get total staked — YOUR ORIGINAL CODE, UNCHANGED
        ...
        // Calculate circulating (total - staked) — YOUR ORIGINAL CODE, UNCHANGED
        ...
        // ────────────────────────────────────────────────────────────────
        // ADDED: Load totalHolders (non-blocking, after main logic)
        // ────────────────────────────────────────────────────────────────
        const holdersEl = document.getElementById('holders');
        ...
        tokenContract.totalHolders()
            .then(holdersRaw => {
                console.log('Holders loaded:', holdersRaw.toString());
                if (holdersEl) {
                    holdersEl.textContent = holdersRaw.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") || '0';
                }
            })
            .catch(err => {
                console.error('Failed to load holders:', err.message);
                if (holdersEl) holdersEl.textContent = 'N/A';
            });
        ...
        // Set fallback values — YOUR ORIGINAL CODE, UNCHANGED
