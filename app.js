...
async function loadHomeData() {
    ...
    if (circEl) circEl.textContent = 'Loading...';

    // Load staked + circulating FIRST (your original working logic, no change)
    if (stakingContract) {
        stakingContract.getTotalStaked()
            .then(totalStaked => {
                console.log('Total staked success:', totalStaked.toString());
                if (stakedEl) stakedEl.textContent = Utils.formatTokenAmount(totalStaked);

                const totalBn = ethers.utils.parseEther('2500000');
                const circulating = totalBn.sub(totalStaked);
                if (circEl) circEl.textContent = Utils.formatTokenAmount(circulating);
            })
            .catch(err => {
                console.error('Staked load error:', err.message);
                if (stakedEl) stakedEl.textContent = 'N/A';
                if (circEl) circEl.textContent = 'N/A';
            });
    } else {
        if (stakedEl) stakedEl.textContent = 'N/A';
        if (circEl) circEl.textContent = 'N/A';
    ...
    // Holders fetch – isolated, won't break anything else
    if (tokenContract) {
        console.log('Attempting holders fetch...');
        tokenContract.totalHolders()
            .then(raw => {
                console.log('Holders success:', raw.toString());
                if (holdersEl) {
                    holdersEl.textContent = raw.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") || '0';
                }
            })
            .catch(err => {
                console.error('Holders fetch error:', err.message);
