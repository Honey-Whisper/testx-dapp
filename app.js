...
async function loadHomeData() {
    ...
    // Yeh forced N/A hata do ya comment kar do
    ...
    try {
        ...
        if (totalSupplyEl) totalSupplyEl.textContent = '2,500,000';

        // Total Staked (purana code same)
        ...
        if (totalStakedEl) totalStakedEl.textContent = Utils.formatTokenAmount(totalStaked);
        ...
        if (circulatingSupplyEl) circulatingSupplyEl.textContent = Utils.formatTokenAmount(circulating);

        // Holders fetch – yeh add karo (non-blocking)
        console.log('Trying to fetch holders...');
        tokenContract.totalHolders()
            .then(holdersRaw => {
                console.log('Holders count:', holdersRaw.toString());
                const holdersEl = document.getElementById('holders');
                if (holdersEl) {
            ...
            .catch(err => {
                console.error('Holders fetch fail:', err.message);
                const holdersEl = document.getElementById('holders');
                if (holdersEl) holdersEl.textContent = 'N/A';
            });
