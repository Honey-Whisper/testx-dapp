...
async function loadHomeData() {
    ...
    if (totalSupplyEl) totalSupplyEl.textContent = '2,500,000'; // quick static
    ...
    try {
        if (!tokenContract || !stakingContract) {
            ...
            if (holdersEl) holdersEl.textContent = 'N/A (no contract)';
            return;
        }

        // Run fetches in parallel + with timeout to prevent freeze
        const timeoutPromise = (ms) => new Promise((_, reject) => 
            setTimeout(() => reject(new Error('RPC timeout')), ms)
        );

        // Holders fetch with timeout
        const holdersPromise = Promise.race([
            tokenContract.totalHolders(),
            timeoutPromise(5000)  // 5 sec max
        ]).then(holdersRaw => {
            console.log('Total holders:', holdersRaw.toString());
            if (holdersEl) {
                holdersEl.textContent = holdersRaw.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
            }
        }).catch(err => {
            console.error('Holders fetch failed:', err);
            if (holdersEl) holdersEl.textContent = 'N/A';
        });

        // Total staked fetch with timeout
        const stakedPromise = Promise.race([
            ...
            // Circulating
            const totalSupplyBn = ethers.utils.parseEther('2500000');
            const circulating = totalSupplyBn.sub(totalStaked);
            if (circulatingSupplyEl) {
                circulatingSupplyEl.textContent = Utils.formatTokenAmount(circulating);
            }
        }).catch(err => {
            console.error('Staked fetch failed:', err);
            if (totalStakedEl) totalStakedEl.textContent = 'Error';
            if (circulatingSupplyEl) circulatingSupplyEl.textContent = 'Error';
        });

        // Wait for both (but UI stays responsive since async)
        await Promise.all([holdersPromise, stakedPromise]);

        console.log('Home data loaded');

    } catch (error) {
        console.error('General home data error:', error);
        if (holdersEl) holdersEl.textContent = 'N/A (error)';
        if (totalStakedEl) totalStakedEl.textContent = 'Error';
        if (circulatingSupplyEl) circulatingSupplyEl.textContent = 'Error';
    }
}
