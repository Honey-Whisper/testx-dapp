...
const CONTRIBUTION_ABI = [
    ...
    'function getUserContribution(address) view returns (uint256 tokens, uint256 eth)',
...
class ContributionContract {
    ...
    async getUserContribution(address) {
        try {
            ...
            return { tokens: ethers.BigNumber.from(0), eth: ethers.BigNumber.from(0) };
        }
    }
    ...
    splitTokens(totalTokens) {
        const instant = totalTokens * 0.7; // 70%
        const staked = totalTokens * 0.3;  // 30%
